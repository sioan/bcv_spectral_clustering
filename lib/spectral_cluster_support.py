...
def get_laplacian(adjacency, n_components=8, eigen_solver=None,
                       random_state=None, eigen_tol=0.0,
                       norm_laplacian=True, drop_first=True):
    """Project the sample on the first eigenvectors of the graph Laplacian.
    The adjacency matrix is used to compute a normalized graph Laplacian
    ...
    * Toward the Optimal Preconditioned Eigensolver: Locally Optimal
    ...
    embedding = _deterministic_vector_sign_flip(embedding)
    if drop_first:
        return laplacian, embedding[1:n_components].T
    else:
        return laplacian, embedding[:n_components].T
...
def get_number_of_clusters(X,n_iterations,min_n_clusters,max_n_clusters,regularization_parameter,affinity,gamma):
    ...
    laplacian,my_embedding = get_laplacian(spectral.affinity_matrix_,
...
def get_number_of_clusters_fast(X,n_iterations,min_n_clusters,max_n_clusters,regularization_parameter,affinity,gamma):
    ...
    for j in np.arange(n_interations):
        ...
        #MSE_list.append(temp_MSE_list)
        temp_MSE_list = score_fast(my_matrix,
                                       k=k,
                                       d_n_rows=int(y_stack.shape[0]/2),
                                       d_n_cols=int(y_stack.shape[1]/2),
                                       SVD_package='numpy')[:,1]
